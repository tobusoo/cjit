...
cl::opt<unsigned> IterationDuration(
        "iter-duration",
        cl::desc("Number of seconds to run each iteration"),
        cl::cat(TestRunnerCat),
        cl::init(1));

cl::opt<unsigned> Iterations(
        "iters",
        cl::desc("Number of iterations to run each benchmark"),
        cl::cat(TestRunnerCat),
        cl::init(10));
...
static std::unique_ptr<llvm::Module> parseIRFromFile(LLVMContext& Ctx, std::filesystem::path P)
{
    SMDiagnostic Err;
...
TestRunner::TestRunner(std::vector<fs::path>&& Benchmarks)
    : Benchmarks(std::move(Benchmarks)), JIT(ExitOnErr(LLJITBuilder().create()))
{
}
