...
static cl::opt<std::string> BenchmarksDir(
        "benchmarks-dir",
        cl::desc("Path to directory with benchmarks"),
        cl::cat(TestRunnerCat),
        cl::Optional);

static cl::opt<std::string> Benchmark(
        "benchmark",
        cl::desc("Path to specific benchmark directory"),
        cl::cat(TestRunnerCat),
        cl::Optional);
...
static std::vector<fs::path> findBenchmarksToRun()
{
    ...
    auto FindSingleBenchmark = [](fs::path Dir) -> std::optional<fs::path> {
        ...
        if (!fs::exists(IRPath)) {
            errs() << "Directory " << Dir << " doesn't contain " << IRName << ", skipping.\n";
            return std::nullopt;
        }
    ...
    if (Benchmark.getNumOccurrences()) {
        if (!fs::exists(Benchmark.getValue()) || !fs::is_directory(Benchmark.getValue())) {
            errs() << "Benchmark dir " << Benchmark << " doesn't exist or is not a directory\n";
            return {};
        }
    ...
    if (!fs::exists(BenchmarksDir.getValue()) || !fs::is_directory(BenchmarksDir.getValue())) {
        errs() << "Benchmarks dir " << BenchmarksDir << " doesn't exist or is not a directory\n";
        return {};
    }
