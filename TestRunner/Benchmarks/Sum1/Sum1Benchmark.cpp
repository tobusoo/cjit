...
std::optional<double> Sum1Benchmark::run(unsigned NumIters, unsigned IterLength)
{
    FunctionExecutor Exec(std::move(TheModule), std::move(Ctx), JIT);

    auto Sum1 = Exec.lookupFunc<unsigned()>("sum_1");
    ...
    auto IterJob = [&]() {
        unsigned Res = Sum1();
        if (Res != ExpectedRes) {
            errs() << "Got unexpected result: expected " << llvm::format("%u", ExpectedRes)
                   << ", got " << llvm::format("%u", Res) << "\n";
            return false;
        }
        return true;
    };
