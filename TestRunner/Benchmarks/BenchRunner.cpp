...
double BenchRunner::runIter(unsigned IterNum, unsigned Duration, llvm::function_ref<bool()> IterJob)
{
    outs() << "Iteration #" << IterNum << ": ";
    using ClockTy = std::chrono::high_resolution_clock;
    auto TimeLimit = std::chrono::seconds(Duration);
    auto TimeLimitS = std::chrono::duration_cast<std::chrono::seconds>(TimeLimit).count();
